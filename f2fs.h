	...
	do { \
		if (unlikely(condition)) { \
        	...
        	{ \
        		...
        		dsm_client_notify(f2fs_dclient, DSM_F2FS_BUGCHK_NEED_FSCK); \
        	} \
		} \
		BUG_ON(condition); \
	} while (0)
...
struct extent_tree {
	...
	struct list_head list;		/* to be userd by sbi->zombie_list */
...
static inline void set_sbi_flag(struct f2fs_sb_info *sbi, unsigned int type)
{
	...
	if (type == SBI_NEED_FSCK) {
		...
		if (f2fs_dclient && !dsm_client_ocuppy(f2fs_dclient)) {
			dsm_client_record(f2fs_dclient, "F2FS-fs: super need fsck\n");
			dsm_client_notify(f2fs_dclient, DSM_F2FS_SUPER_NEED_FSCK);
		}
	}
#endif
}
